...
pub fn cancel_confirmation_text(details: &TaskStatusDetails) -> String {
    format!(
        "⛔ Подтвердите отмену\n\n\
         Действительно отменить задачу {} «{}»?\n\
         Действие увидят все участники задачи.",
...
pub fn task_comment_prompt(details: &TaskStatusDetails) -> String {
    format!(
        "💬 Комментарий к задаче {} «{}»\n\n\
         Отправьте один короткий комментарий. Он появится в карточке \
         и уйдёт участникам задачи.",
        details.public_code, details.title
    )
}
...
pub fn task_blocker_prompt(details: &TaskStatusDetails) -> String {
    format!(
        "🚧 Что мешает по задаче {} «{}»?\n\n\
         Опишите блокер одним сообщением. Я отмечу задачу как заблокированную \
         и оповещу автора и менеджера.",
        details.public_code, details.title
    )
}
...
pub fn task_reassign_prompt(details: &TaskStatusDetails) -> String {
    format!(
        "👤 Кому передать задачу {} «{}»?\n\n\
         ...
         Если совпадение будет неочевидным — я попрошу уточнить.",
        details.public_code, details.title
    )
}
...
pub fn delivery_help_text(details: &TaskStatusDetails) -> String {
    ...
    format!(
        "👋 Как подключить исполнителя\n\n\
         Задача {} уже сохранена, но {assignee} ещё не запускал этого бота.\n\n\
         Что делать дальше:\n\
         ...
         «Открой, пожалуйста, бота для задач и отправь команду /start. \
         После этого я смогу назначать тебе задачи прямо в Telegram.»",
        details.public_code
    )
}
