...
pub fn admin_menu_text(actor: &User) -> String {
    ...
    format!(
        "🛡 Панель администратора\n\n\
         ...
         • Журнал безопасности — отказы в доступе и подозрительные события.\n\
...
pub fn admin_access_denied_text() -> String {
    format!(
        "{INFO_EMOJI} Доступ к панели администратора ограничен.\n\
...
pub fn admin_self_target_text() -> String {
    format!(
        "{INFO_EMOJI} Эту операцию нельзя выполнить над собой.\n\
         Попросите другого администратора выполнить её — это правило защищает систему \
         от ситуации, когда не остаётся ни одного активного администратора."
    )
}

pub fn admin_last_admin_text() -> String {
    format!(
        "{INFO_EMOJI} Операцию нельзя завершить: вы — последний активный администратор.\n\
         Сначала повысьте другого пользователя до роли «Администратор», \
...
pub fn admin_security_audit_text(entries: &[SecurityAuditEntry]) -> String {
    ...
    format!(
        "🔐 Журнал безопасности (последние {count})\n\n{body}\n\n\
         ⚠️ Здесь фиксируются отказы в доступе, попытки нажать чужие кнопки подтверждения \
         и срабатывания ограничения частоты запросов.",
        count = entries.len(),
        body = lines.join("\n")
    )
}
...
/// Человекочитаемое название роли в описательном тексте (внутри предложения,
/// после двоеточия и т.п.).  Намеренно строчное и в полной форме — стилевой
/// якорь зафиксирован в `docs/ux-style-guide.md`, раздел «Терминология».
///
/// Для подписей кнопок (где нужно покороче и в форме «команды») используется
/// отдельный набор констант рядом с keyboard'ом.
